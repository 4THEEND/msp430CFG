...
public:
    ...
    virtual std::set<uint32_t> get_next_addrs(State& state){ return {address + 2 * instruction_length }; };
...
public:
    ...
    std::set<uint32_t> get_next_addrs(State& state) override;
...
public:
    ...
    std::set<uint32_t> get_next_addrs(State& state) override { return {}; };
...
public:
    ...
    std::shared_ptr<Instruction> Build(uint32_t instruction_address, std::shared_ptr<BinaryLoader> binary_file) const {
